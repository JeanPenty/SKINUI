...
//数组模板类
template <class TYPE, class ARG_TYPE=const TYPE &> class GAME_ENGINE_TEMPLATE cCArray
{
	//变量定义
protected:
	TYPE *							m_pData;							//数组指针
...
public:
	...
	//拷贝数组
	VOID Copy(const cCArray & Src);
	//追加数组
	INT_PTR Append(const cCArray & Src);
	//获取元素
...
public:
	...
	//插入数据
	VOID InsertAt(INT_PTR nIndex, const cCArray & Src);
	//插入数据
...
bool cCArray<TYPE, ARG_TYPE>::IsEmpty() const
{
...
//获取数目
template<class TYPE, class ARG_TYPE>
INT_PTR cCArray<TYPE, ARG_TYPE>::GetCount() const
{
...
//增加元素
template<class TYPE, class ARG_TYPE>
INT_PTR cCArray<TYPE,ARG_TYPE>::Add(ARG_TYPE newElement)
{
...
//操作重载
template<class TYPE, class ARG_TYPE>
TYPE & cCArray<TYPE, ARG_TYPE>::operator[](INT_PTR nIndex)
{ 
...
//操作重载
template<class TYPE, class ARG_TYPE>
const TYPE & cCArray<TYPE, ARG_TYPE>::operator[](INT_PTR nIndex) const
{ 
	return GetAt(nIndex);
}
...
//获取缓冲
template<class TYPE, class ARG_TYPE> 
TYPE * cCArray<TYPE,ARG_TYPE>::GetData()
{
	return m_pData;
}
...
//获取缓冲
template<class TYPE, class ARG_TYPE> 
const TYPE * cCArray<TYPE,ARG_TYPE>::GetData() const
{
	return m_pData;
}
...
//拷贝数组
template<class TYPE, class ARG_TYPE> 
VOID cCArray<TYPE,ARG_TYPE>::Copy(const cCArray & Src)
{
	//效验参数
	assert(this!=&Src);
	if (this==&Src) return;
...
//追加数组
template<class TYPE, class ARG_TYPE> 
INT_PTR cCArray<TYPE,ARG_TYPE>::Append(const cCArray & Src)
{
	//效验参数
	assert(this!=&Src);
...
//获取元素
template<class TYPE, class ARG_TYPE> 
TYPE & cCArray<TYPE,ARG_TYPE>::GetAt(INT_PTR nIndex)
{
	assert((nIndex>=0)&&(nIndex<m_nElementCount));
	if ((nIndex<0)||(nIndex>=m_nElementCount)) throw 0;//AfxThrowInvalidArgException();
...
//获取元素
template<class TYPE, class ARG_TYPE> 
const TYPE & cCArray<TYPE,ARG_TYPE>::GetAt(INT_PTR nIndex) const
{
	assert((nIndex>=0)&&(nIndex<m_nElementCount));
	if ((nIndex<0)||(nIndex>=m_nElementCount)) throw 0;//AfxThrowInvalidArgException();
...
//获取元素
template<class TYPE, class ARG_TYPE> 
TYPE & cCArray<TYPE,ARG_TYPE>::ElementAt(INT_PTR nIndex)
{
	assert((nIndex>=0)&&(nIndex<m_nElementCount));
	if ((nIndex<0)&&(nIndex>=m_nElementCount)) throw 0;//AfxThrowInvalidArgException();
...
//获取元素
template<class TYPE, class ARG_TYPE> 
const TYPE & cCArray<TYPE,ARG_TYPE>::ElementAt(INT_PTR nIndex) const
{
	assert((nIndex>=0)&&(nIndex<m_nElementCount));
...
//设置大小
template<class TYPE, class ARG_TYPE>
VOID cCArray<TYPE,ARG_TYPE>::SetSize(INT_PTR nNewSize)
{
	//效验参数
	assert(nNewSize>=0);
	if (nNewSize<0)	throw 0;//AfxThrowInvalidArgException();
...
//设置元素
template<class TYPE, class ARG_TYPE>
VOID cCArray<TYPE,ARG_TYPE>::SetAt(INT_PTR nIndex, ARG_TYPE newElement)
{
	assert((nIndex>=0)&&(nIndex<m_nElementCount));
	if ((nIndex>=0)&&(nIndex<m_nElementCount)) m_pData[nIndex]=newElement;
	else throw 0;//AfxThrowInvalidArgException();
...
//设置元素
template<class TYPE, class ARG_TYPE>
VOID cCArray<TYPE,ARG_TYPE>::SetAtGrow(INT_PTR nIndex, ARG_TYPE newElement)
{
	//效验参数
	assert(nIndex>=0);
	if (nIndex<0) throw 0;//AfxThrowInvalidArgException();
...
//插入数据
template<class TYPE, class ARG_TYPE>
VOID cCArray<TYPE,ARG_TYPE>::InsertAt(INT_PTR nIndex, const cCArray & Src)
{
	//效验参数
	assert(nStartIndex>=0);
	if (nStartIndex<0) throw 0;//AfxThrowInvalidArgException();
...
//插入数据
template<class TYPE, class ARG_TYPE>
VOID cCArray<TYPE,ARG_TYPE>::InsertAt(INT_PTR nIndex, ARG_TYPE newElement, INT_PTR nCount)
{
	//效验参数
	assert(nIndex>=0);
	assert(nCount>0);
	if ((nIndex<0)||(nCount<=0)) throw 0;//AfxThrowInvalidArgException();
...
//删除数据
template<class TYPE, class ARG_TYPE>
VOID cCArray<TYPE,ARG_TYPE>::RemoveAt(INT_PTR nIndex, INT_PTR nCount)
{
	//效验参数
	assert(nIndex>=0);
	assert(nCount>=0);
	assert(nIndex+nCount<=m_nElementCount);
	if ((nIndex<0)||(nCount<0)||((nIndex+nCount>m_nElementCount))) throw 0;//AfxThrowInvalidArgException();
...
//删除元素
template<class TYPE, class ARG_TYPE>
VOID cCArray<TYPE,ARG_TYPE>::RemoveAll()
{
	if (m_nElementCount>0)
	{
		for (INT_PTR i=0;i<m_nElementCount;i++) (m_pData+i)->~TYPE();
...
//释放内存
template<class TYPE, class ARG_TYPE>
VOID cCArray<TYPE,ARG_TYPE>::FreeMemory()
{
...
//申请内存
template<class TYPE, class ARG_TYPE>
VOID cCArray<TYPE,ARG_TYPE>::AllocMemory(INT_PTR nNewCount)
{
	//效验参数
	assert(nNewCount>=0);
