...
bool CVirtualWindow::ActiveWindow( cCRect & rcWindow, DWORD dwStyle, UINT uWindowID, CVirtualEngine * pVirtualEngine, CVirtualWindow * pParentWindow )
{
...
VOID CVirtualWindow::GetClientRect( cCRect & rcClient )
{
	rcClient.SetRect(0,0,m_WindowSize.cx,m_WindowSize.cy);
}

VOID CVirtualWindow::GetWindowRect( cCRect & rcWindow )
{
...
VOID CVirtualWindow::SetWindowPos( INT nXPos, INT nYPos, INT nWidth, INT nHeight, UINT uFlags )
{
	...
	if ((uFlags & (SWP_NOMOVE|SWP_NOSIZE)) != (SWP_NOMOVE|SWP_NOSIZE))
	{
		...
		cCRect rectOld(m_BenchmarkPos.x,m_BenchmarkPos.y,m_BenchmarkPos.x+m_WindowSize.cx,m_BenchmarkPos.y+m_WindowSize.cy);
...
// 	uFlags |= (SWP_NOMOVE|SWP_NOSIZE);
	...
	for (INT_PTR i=0;i<m_VirtualWindowPtrArray.GetCount();i++)
	{
		...
		cCPoint pt(pVirtualWindow->m_BenchmarkPos.x+nXPos-xOld,pVirtualWindow->m_BenchmarkPos.y+nYPos-yOld);
